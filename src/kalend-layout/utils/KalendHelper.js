...
const getEventDateTime = (event, config) => {
    const dateTimeStart = (0, LuxonHelper_1.parseToDateTime)(event.startAt, event.timezoneStartAt || config.timezone);
    const dateTimeEnd = (0, LuxonHelper_1.parseToDateTime)(event.endAt, event.timezoneStartAt || config.timezone);
    return {
        dateTimeStart,
        dateTimeEnd,
    };
